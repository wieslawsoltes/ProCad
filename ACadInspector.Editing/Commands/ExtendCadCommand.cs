...
public sealed class ExtendCadCommand : ICadCommandHandler
{
    public string Name => "EXTEND";
    public IReadOnlyList<string> Aliases => ["EX"];
    ...
    public ValueTask<CadCommandResult> ExecuteAsync(CadCommandContext context)
    {
        ...
        {
            if (!CadTrimExtendGeometry.TryComputeExtendedLine(targetLine, boundaryEntity, endpoint, out var newStart, out var newEnd, out var geometryError))
            {
            ...
            return ValueTask.FromResult(CadCommandResult.Ok("Extend completed.", [forwardOperation]));
        }
        ...
        {
            return ValueTask.FromResult(CadCommandResult.Fail("EXTEND target polyline must be open."));
        }

        if (!CadPolylineEditValidation.TryValidateLinearPolyline(targetPolyline, "EXTEND", out var unsupportedError))
        {
        ...
        {
            return ValueTask.FromResult(CadCommandResult.Fail("EXTEND target polyline must have at least two vertices."));
        }

        if (!TryExtendPolylineEndpoint(fromVertices, boundaryEntity, endpoint, out var toVertices, out var polylineError))
        {
        ...
        return ValueTask.FromResult(CadCommandResult.Ok("Extend completed.", [forwardPolyline]));
    }

    private static bool TryExtendPolylineEndpoint(
        ...
        out string? error)
    {
        ...
        if (!CadTrimExtendGeometry.TryComputeExtendedLine(
                targetSegment,
                boundaryEntity,
                endpoint: CadLineEndpoint.End,
                out _,
                out var extendedEndpoint,
                out error))
        {
            return false;
        }

        vertices[endpointIndex] = extendedEndpoint;
        toVertices = vertices;
        return true;
    }
}
