...
public sealed class MoveCadCommand : ICadCommandHandler
{
    ...
    public ValueTask<CadCommandResult> ExecuteAsync(CadCommandContext context)
    {
        ...
        foreach (var target in targets)
        {
            ...
            switch (target)
            {
                case Line line:
                {
                    ...
                    var toStart = CadGeometryTransform.Translate(fromStart, delta);
                    var toEnd = CadGeometryTransform.Translate(fromEnd, delta);
                ...
                case Arc arc:
                {
                    ...
                    var toCenter = CadGeometryTransform.Translate(fromCenter, delta);
                ...
                case LwPolyline polyline:
                {
                    ...
                    var toVertices = CadGeometryTransform.TranslateVertices(fromVertices, delta);
                ...
                case Spline spline:
                {
                    ...
                    var toFitPoints = fromFitPoints
                        .Select(point => CadGeometryTransform.Translate(point, delta))
                        .ToArray();
                    ...
                    var toStartTangent = fromStartTangent;
                    var toEndTangent = fromEndTangent;
                    var toNormal = fromNormal;
                ...
                case Hatch hatch:
                {
                    ...
                    {
                        return ValueTask.FromResult(CadCommandResult.Fail($"MOVE could not transform HATCH: {loopError}"));
                    }

                    var toLoops = CadHatchGeometry.TranslateLoops(fromLoops, delta);
                ...
                case Insert insert:
                {
                    ...
                    {
                        return ValueTask.FromResult(CadCommandResult.Fail("MOVE cannot transform INSERT without a block reference."));
                    }
                ...
                default:
                    return ValueTask.FromResult(CadCommandResult.Fail($"MOVE does not support entity type '{target.GetType().Name}' yet."));
            }
        }
        ...
        return ValueTask.FromResult(CadCommandResult.Ok($"Moved {targets.Count} entity(s).", forward));
    }
}
