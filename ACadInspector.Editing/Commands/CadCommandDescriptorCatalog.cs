...
internal static class CadCommandDescriptorCatalog
{
    private static readonly IReadOnlyDictionary<string, DescriptorTemplate> Templates =
        new Dictionary<string, DescriptorTemplate>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["TRIM"] = T(
                "Trims target geometry to a boundary.",
                ...
                [K("START"), K("END")]),
            ["EXTEND"] = T(
                "Extends target geometry to a boundary.",
                "EXTEND boundaryHandle targetHandle [START|END]",
                [P("boundaryHandle", CadCommandParameterKind.Handle), P("targetHandle", CadCommandParameterKind.Handle), P("side", CadCommandParameterKind.Keyword, optional: true)],
                [K("START"), K("END")]),
            ["BREAK"] = T(
                "Breaks line geometry at a point.",
                "BREAK targetHandle breakPoint",
                [P("targetHandle", CadCommandParameterKind.Handle), P("breakPoint", CadCommandParameterKind.Coordinate)]),
            ["JOIN"] = T(
                "Joins compatible entities into one result.",
                "JOIN handle1 handle2",
                [P("handle1", CadCommandParameterKind.Handle), P("handle2", CadCommandParameterKind.Handle)]),
            ["FILLET"] = T(
                "Creates tangent arc between two lines.",
                "FILLET radius [line1 line2]",
                [P("radius", CadCommandParameterKind.Distance), P("line1", CadCommandParameterKind.Handle, optional: true), P("line2", CadCommandParameterKind.Handle, optional: true)]),
            ["CHAMFER"] = T(
                "Creates chamfer between two lines.",
                "CHAMFER distance1 [distance2] [line1 line2]",
                [P("distance1", CadCommandParameterKind.Distance), P("distance2", CadCommandParameterKind.Distance, optional: true), P("line1", CadCommandParameterKind.Handle, optional: true), P("line2", CadCommandParameterKind.Handle, optional: true)]),
            ["ARRAY"] = T(
