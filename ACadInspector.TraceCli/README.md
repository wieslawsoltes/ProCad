...
```bash
dotnet run --project ACadInspector.TraceCli -- --input /path/to/file.dxf
```

Run multiple files in one pass:

```bash
dotnet run --project ACadInspector.TraceCli -- \
  --input /path/to/a.dxf \
  --input /path/to/b.dwg
```
...
```bash
dotnet build ACadInspector.TraceCli/ACadInspector.TraceCli.csproj -c Release

dotnet-trace collect \
  --output trace-output/sample.nettrace \
  -- \
  /usr/local/share/dotnet/dotnet \
  ACadInspector.TraceCli/bin/Release/net10.0/ACadInspector.TraceCli.dll \
  --warmup 1 \
  --iterations 20 \
  --no-image \
